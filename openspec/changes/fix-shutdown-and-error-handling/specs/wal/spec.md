...
flusher signals after fsync completes. If a flush operation fails, the flusher
SHALL signal all affected callbacks before transitioning to read-only mode, so
that no caller blocks indefinitely. Callers requiring synchronous durability
...
#### Scenario: Callbacks signaled on flush failure
- **WHEN** the flusher encounters an IO error during write or fsync
- **THEN** all durability callbacks in the failed batch are signaled
- **AND** no caller blocks indefinitely on an unsignaled callback
- **AND** the flusher transitions the database to read-only mode
...
replayed `InsertRow` entries. Duplicate row IDs within the same table SHALL be
rejected as a fatal error. Corrupted trailing entries (from incomplete writes)
SHALL be truncated and the caller SHALL receive a list of warning messages
describing the truncated data rather than silently discarding it.
...
- **AND** the warning is returned to the caller as part of the replay result
...
#### Scenario: Replay rejects duplicate row IDs
- **WHEN** a WAL file contains two InsertRow entries for the same table with the same row ID
- **AND** replay is performed
- **THEN** a fatal error is returned indicating the duplicate row ID
- **AND** replay does not silently overwrite the existing row
...
read-only error state and signal the flusher completion signal so that shutdown
does not hang. On clean shutdown, the flusher SHALL drain the TBQueue fully,
fsync, and signal the completion signal. Flush errors during the shutdown drain
SHALL be propagated via the completion signal rather than silently discarded.
...
- **AND** the flusher completion signal is set so shutdown does not deadlock
...
- **AND** signals the completion signal
- **AND** closes the WAL file handle

#### Scenario: Shutdown with flush errors
- **WHEN** the database is shut down gracefully
- **AND** an IO error occurs during the final drain flush
- **THEN** the error is communicated via the completion signal
- **AND** closeDatabase can detect and propagate the error
